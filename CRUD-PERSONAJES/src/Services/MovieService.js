    ...
    getMovie = async (titulo, order) => {
        ...
        if(!titulo){
            if(order=="DESC"){
                ...
                .query(`SELECT * from ${MovieTabla} ORDER BY ${MovieTabla}.fechacreacion DESC`)
            }
            else if(order=="ASC"){
                response = await pool.request()
                .query(`SELECT * from ${MovieTabla} ORDER BY ${MovieTabla}.fechacreacion ASC`)
            }
            else{
                response = await pool.request()
            ...
            if(order=="ASC"){
                ...
                .query(`SELECT * from ${MovieTabla} WHERE titulo=@Titulo ORDER by Pelicula.fechacreacion ASC`);
            }
            else if(order=="DESC"){
                response = await pool.request()
                .input('Titulo', sql.VarChar, titulo)
                .query(`SELECT * from ${MovieTabla} WHERE titulo=@Titulo ORDER by Pelicula.fechacreacion DESC`);
            }
            else{
                response = await pool.request()
                .input('Titulo', sql.VarChar, titulo)
